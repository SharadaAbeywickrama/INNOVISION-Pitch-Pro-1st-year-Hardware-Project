...
unsigned long lastCorrectionMove = 0; // Track last correction movement
float deadbandZone = 2.0; // Dead zone where no correction is applied
bool inDeadband = false; // Flag to indicate we're in acceptable range
int consecutiveDeadbandReadings = 0; // Count readings in deadband
// INTELLIGENT CORRECTION VARIABLES (non-static)
...
if (turnFirstAttempt) {
    int turnSpeed = 75;
    float stopThreshold = 70.0;
    
   if (turningRate > 30) {
  stopThreshold = 66.0;  // Much earlier
} else if (turningRate > 20) {
  stopThreshold = 68.0;  // Earlier
} else if (turningRate > 10) {
  stopThreshold = 70.0;  // Slightly earlier
}
    if (turnDirection == "right") {
    ...
    // ===== SECOND ATTEMPT: Intelligent one-shot correction with verification =====
    ...
    // INTELLIGENT TWO-STAGE ONE-SHOT CORRECTION (using global variables)
    
    if (!correctionApplied) {
      // STAGE 1: APPLY FIRST CORRECTION
      ...
      if (correctionAttempts > 2) {
        if (correctionAttempts == 2) errorFromTarget *= 0.5;
        Serial.println("⚠ MAX CORRECTIONS REACHED - Finalizing");
        finalizeTurn();
        return;
      }
      
      // Calculate ADAPTIVE correction based on error size
      float correctionSpeed;
      unsigned long correctionDuration;
      
      if (abs(errorFromTarget) > 12.0) {
        correctionSpeed = 70;
        correctionDuration = 1000; // 1 second for large errors
      } else if (abs(errorFromTarget) > 8.0) {
        correctionSpeed = 65;
        correctionDuration = 800;
      } else if (abs(errorFromTarget) > 5.0) {
        correctionSpeed = 58;
        correctionDuration = 650;
      } else if (abs(errorFromTarget) > 3.0) {
        correctionSpeed = 50;
        correctionDuration = 500;
      } else {
        correctionSpeed = 42;
        correctionDuration = 350;
      }
      
      Serial.println("===== APPLYING INTELLIGENT ONE-SHOT CORRECTION =====");
      ...
      // STAGE 2: CORRECTION APPLIED - Wait for completion then verify
      if (millis() >= correctionCompleteTime) {
        stopMotors();
        Serial.println("===== CORRECTION APPLIED - VERIFYING RESULT =====");
        verificationDone = true;
        verificationCompleteTime = millis() + 1000; // 1 second for settling and verification
      }
      
    } else {
      // STAGE 3: VERIFICATION PERIOD - Check if we need another correction
      if (millis() >= verificationCompleteTime) {
        // Take fresh readings for verification
        ...
        if (abs(verifiedError) <= 3.0 || correctionAttempts >= 2) {
          // Success or maximum attempts reached
          ...
          Serial.println("🔄 NEEDS ADDITIONAL CORRECTION - Preparing second shot");
    ...
    // SAFETY TIMEOUT - Extended for verification process
  ...
  // Optimized debug output (back to working version)
  static unsigned long lastDebug = 0;
...
void startCorrectionAttempt(float error, float currentHeading) {
  ...
  // Reset correction variables (now global, not static)
  correctionApplied = false;
  verificationDone = false;
  ...
  Serial.println("===== STARTING INTELLIGENT ONE-SHOT CORRECTION =====");
  ...
  Serial.println("🧠 INTELLIGENT: One-shot correction with verification & adaptive second shot");
}

// Original working oscillation detection (proven to work)
