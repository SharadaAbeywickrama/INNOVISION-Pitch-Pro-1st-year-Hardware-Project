// Arduino Mega - Simple Motor Control
#include <Wire.h>
...
// Speed multipliers for each motor (adjustable)
float leftSpeedFactor = 1.0;
float rightSpeedFactor = 1.0;
...
void loop() {
  if (Serial2.available()) {
    ...
    if (cmd.startsWith("MOVE:")) {
    ...
    else if (cmd.startsWith("FACTORS:")) {
      // Format: FACTORS:left_factor,right_factor (e.g., FACTORS:1.2,0.8)
      int commaPos = cmd.indexOf(',');
      if (commaPos > 0) {
        leftSpeedFactor = cmd.substring(8, commaPos).toFloat();
        rightSpeedFactor = cmd.substring(commaPos + 1).toFloat();
        
        Serial.println("Factors: L=" + String(leftSpeedFactor) + " R=" + String(rightSpeedFactor));
        Serial2.println("FACTORS_SET");
      }
    }
    else if (cmd == "STOP") {
      stopMotors();
      ...
      Serial2.println("STATUS:L=" + String(currentLeftSpeed) + ",R=" + String(currentRightSpeed) + 
                     ",LF=" + String(leftSpeedFactor) + ",RF=" + String(rightSpeedFactor));
    }
  }
...
void moveMotors(int leftSpeed, int rightSpeed) {
  // Apply speed factors
  leftSpeed = leftSpeed * leftSpeedFactor;
  rightSpeed = rightSpeed * rightSpeedFactor;
